...
def get_dof(x, y, nely):
    """
    Devuelve el índice del grado de libertad vertical (uy) del nodo (x, y)
    """
    return [2 * ((nely + 1) * x + y), 2 * ((nely + 1) * x + y) + 1]
...
class MMA:
    ...
    def __init__(self):
        self.epoch = 0;
        #self.initial_move = 0.2       # valor inicial de move
        self.decay_rate = 0.99         # factor de decaimiento
        self.move_min = 0.001
        self.move_decay_iters = 50  # iteraciones para alcanzar move_min
    ...
    def setMoveLimit(self, movelim):
        self.initial_move = movelim #nueva line
        #self.moveLimit = movelim; #vieja linea
    ...
    def mmasub(self, xval):
        ...
        #move = self.moveLimit; #vieja linea
        #move = self.initial_move * (self.decay_rate ** (iter - 1)) #nueva linea exponencial
        
        # nuevas lineas lineal
        #k = iter # empezamos desde 0
        #decay_frac = min(k / self.move_decay_iters, 1.0)
        #move = self.initial_move * (1.0 - decay_frac)
        #move = max(move, self.move_min)
...
def plot_mesh(nelx, nely, show_dofs=True):
    ...
    for ey in range(nely):
        for ex in range(nelx):
            ...
            x = [ex, ex+1, ex+1, ex, ex]
            y = [ey, ey, ey+1, ey+1, ey]
            ax.plot(x, y, 'k-', linewidth=0.5)

            # Element center
            cx, cy = ex + 0.5, ey + 0.5
            elem_id = ex + ey * nelx
            ax.text(cx, cy, f"E{elem_id}", color='blue', ha='center', va='center', fontsize=8)
    ...
    for j in range(nny):
        for i in range(nnx):
            ...
            ax.text(x - 0.1, y + 0.1, f"N{nid}", color='red', fontsize=7)
            if show_dofs:
                dof_x = 2 * nid
                dof_y = 2 * nid + 1
                ax.text(x + 0.05, y - 0.2, f"{dof_x}", color='green', fontsize=6)
                ax.text(x + 0.05, y - 0.35, f"{dof_y}", color='purple', fontsize=6)

    ax.set_aspect('equal')
    ax.set_title(f"Malla {nelx} × {nely}")
    ax.axis('off')
    plt.show()



def plot_and_export_mesh(nelx, nely, filename="mesh.vtu"):
    nnx, nny = nelx + 1, nely + 1
    num_nodes = nnx * nny
    num_elems = nelx * nely

    # --- Coordenadas nodales (formato (num_nodes, 3)) ---
    coords = np.zeros((num_nodes, 3))  # z=0 para 2D
    for j in range(nny):
        for i in range(nnx):
            node_id = i + j * nnx
            coords[node_id, 0] = i
    ...
    # --- Elementos como conectividad de nodos ---
    cells = []
    elem_centers = []
    elem_ids = []
    for ey in range(nely):
        for ex in range(nelx):
            ...
            # Centro de elemento para plot
            cx, cy = ex + 0.5, ey + 0.5
            elem_centers.append((cx, cy))
            elem_ids.append(ex + ey * nelx)

    cells = [("quad", np.array(cells))]

    # --- DOFs por nodo: 2 por nodo (ux, uy) ---
    dof_x = np.arange(0, 2*num_nodes, 2)
    dof_y = np.arange(1, 2*num_nodes, 2)

    # ========== GRAFICAR ==========
    fig, ax = plt.subplots(figsize=(12, 6))
    for e_id, conn in enumerate(cells[0][1]):
        x = coords[conn, 0]
        y = coords[conn, 1]
        x = np.append(x, x[0])
        y = np.append(y, y[0])
        ax.plot(x, y, 'k-', linewidth=0.5)
        cx, cy = elem_centers[e_id]
        ax.text(cx, cy, f"E{e_id}", color='blue', ha='center', va='center', fontsize=8)

    for n in range(num_nodes):
        x, y = coords[n, 0], coords[n, 1]
        ax.plot(x, y, 'ro', markersize=3)
        ax.text(x - 0.1, y + 0.1, f"N{n}", color='red', fontsize=7)
        ax.text(x + 0.05, y - 0.2, f"{2*n}", color='green', fontsize=6)
        ax.text(x + 0.05, y - 0.35, f"{2*n+1}", color='purple', fontsize=6)
    ...
    # ========== EXPORTAR A PARAVIEW ==========
    # Datos por nodo: DOFs (como campos escalar)
    point_data = {
        "dof_x": dof_x.astype(np.float64),
        "dof_y": dof_y.astype(np.float64)
    }

    # Datos por celda (elemento)
    cell_data = {
        "element_id": [np.array(elem_ids)]
    }

    mesh = meshio.Mesh(points=coords, cells=cells,
                       point_data=point_data,
                       cell_data=cell_data)
    mesh.write(filename)
    print(f"✅ Malla exportada como '{filename}' (ábrela en ParaView).")
